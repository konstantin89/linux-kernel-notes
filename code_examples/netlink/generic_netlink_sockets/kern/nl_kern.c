...
int doc_exmpl_echo(struct sk_buff *skb_2, struct genl_info *info) 
{
    ...
    printk("[ ] doc_exmpl_echo() - callback is called\n");
    ...
    printk("[ ] doc_exmpl_echo() - Sending message to user space\n");
    ...
    */
    ...
    if (rc != 0) 
    {
        printk("[X] doc_exmpl_echo() - Failed to send message to user space \n");
        goto out;
    }

    printk("[V] doc_exmpl_echo() - Sent message to user space \n");
